...
def main():
    ...
    if data_args.oscar_data_path:
        ...
        for lg in oscar_langs:
            if lg in ["de", "en", "ru"]:
                data_files = [os.path.join(data_args.oscar_data_path, lg+f"{i}.parquet") for i in range(10)]
            elif lg == "zh":
                data_files = [os.path.join(data_args.oscar_data_path, lg+f"{i}.parquet") for i in range(2)]
            else:
                data_files=os.path.join(data_args.oscar_data_path, lg+".parquet")
